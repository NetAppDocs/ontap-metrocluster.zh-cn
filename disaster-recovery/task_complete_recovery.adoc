...
* 有关 FabricPool 配置的详细信息，请参见 link:https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-psmg/home.html["磁盘和聚合高级指南"]。


.步骤
. 按照中的操作步骤 " 更换 MetroCluster 配置上的 FabricPool 镜像 " 进行操作 link:https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-psmg/home.html["磁盘和聚合高级指南"]。
...
----
+
如果灾难站点包含未镜像聚合，而未镜像聚合不再存在，则该聚合可能会在 storage aggregate show 命令的输出中显示为 "unknown" 状态。请联系技术支持以删除未镜像聚合的过期条目。

. 验证运行正常的集群上的所有 sync-destination SVM 是否均处于休眠状态（显示 Admin State 为 "stopped" ），以及灾难集群上的 sync-source SVM 是否已启动且正在运行：
+
