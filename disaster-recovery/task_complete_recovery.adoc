...
NOTE: 系统会自动生成所需的新许可证密钥，并将其发送到文件中的电子邮件地址。如果您未能在30天内收到包含许可证密钥的电子邮件、请参阅中的"如果我的许可证出现问题、应联系谁？"_部分link:https://kb.netapp.com/Advice_and_Troubleshooting/Flash_Storage/AFF_Series/Post_Motherboard_Replacement_Process_to_update_Licensing_on_a_AFF_FAS_system["NetApp知识库：主板更换后流程、用于更新AFF/FAS系统上的许可。"^]
...
----
+
如果灾难站点包含未镜像聚合，而未镜像聚合不再存在，则该聚合可能会在 storage aggregate show 命令的输出中显示为 "`unknown` " 状态。请联系技术支持以删除未镜像聚合的过期条目、请参考 link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/MetroCluster/How_to_remove_stale_unmirrored_aggregate_entries_in_a_MetroCluster_following_disaster_where_storage_was_lost["NetApp知识库：在存储丢失的灾难发生后、如何在MetroCluster 中删除陈旧的未镜像聚合条目。"^]

. 验证运行正常的集群上的所有 sync-destination SVM 是否均处于休眠状态（显示 Admin State 为 `sstopped` ），以及灾难集群上的 sync-source SVM 是否已启动且正在运行：
+
