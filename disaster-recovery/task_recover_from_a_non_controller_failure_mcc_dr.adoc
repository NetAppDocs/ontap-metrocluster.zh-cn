...
在执行切回之前，请验证 LIF 是否托管在适当的节点和端口上。

.步骤
. 在执行切回之前，请验证 LIF 是否托管在相应的节点和端口上。
+
.. 更改为高级权限级别：
+
...
CAUTION: 在执行切换之前，必须验证数据 LIF 在新节点上的位置是否正确。切换回配置时，ONTAP 会尝试在 LIF 使用的主端口上恢复流量。如果主端口与交换机端口和 VLAN 的连接不正确，可能会发生 I/O 故障。如果在将 LIF 放置在正确的节点或端口上时遇到任何问题，请勿继续执行切换，并联系 NetApp 技术支持。
