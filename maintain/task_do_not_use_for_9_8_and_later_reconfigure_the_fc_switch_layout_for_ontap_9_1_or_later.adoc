...
.步骤
. 使用 ... https://["光纤连接的 MetroCluster 安装和配置"^] 要确定交换机类型的布线布局，请使用八节点 MetroCluster 配置的端口使用情况。
+
...
FC 交换机端口使用情况必须与中所述的使用情况匹配 https://["光纤连接的 MetroCluster 安装和配置"^] 以便可以使用 RCF 。
...
. 使用中所述的协商切换，修复和切回过程 https://["MetroCluster 管理和灾难恢复"^]。
