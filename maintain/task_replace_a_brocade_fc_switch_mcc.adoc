...
+
.. 在运行正常的交换机上为每个端口禁用 ISL 端口： + ` * portcfgpersistentdisable _port-number_*`
+
[listing]
----
...
----
.. 如果受损交换机仍在运行，请为每个端口禁用该交换机上的 FC-VI 和 HBA 端口： + ` * portcfgpersistentdisable _port-number_*`
+
[listing]
----
...
. 如果受损交换机仍在运行，请从 ` * switchshow*` 命令收集输出。
+
[listing]
----
...
+
.. 启动新交换机并让其启动。
.. 检查交换机上的固件版本以确认其与其他 FC 交换机的版本匹配： + ` * FirmwareShow *`
.. 按照 _Brocade 安装和配置指南 _ 中所述配置新交换机，但跳过 "`在 MetroCluster FC 交换机上配置分区` " 一节。
+
...
. 如果要启用 ISL 加密，请完成中的适用任务 link:https://docs.netapp.com/us-en/ontap-metrocluster/install-fc/index.html["《光纤连接的 MetroCluster 安装和配置指南》"]。
+
...
. 完成新交换机的配置：
+
.. 启用 ISL ： + ` * 端口 cfgpersistentenable _port-number_*`
+
[listing]
----
...
----
.. 在替代交换机（示例中为 FC_switch_B_1 ）上，验证 ISL 是否联机： + ` * switchshow*`
+
[listing]
----
FC_switch_B_1:admin> switchshow
