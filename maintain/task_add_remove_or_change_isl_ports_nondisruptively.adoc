...
NOTE: 如果要在连接 NS224 磁盘架的共享配置中使用 Cisco 9336C-FX2 交换机，更改 ISL 可能需要新的参考配置文件 (RCF)。如果您当前和新的 ISL 速度为 40 Gbps 和 100 Gbps，则不需要新的 RCF。对 ISL 速度的所有其他更改都需要新的 RCF。例如，将 ISL 速度从 40 Gbps 更改为 100 Gbps 不需要新的 RCF，但将 ISL 速度从 10 Gbps 更改为 40 Gbps 则需要新的 RCF。
...
NOTE: 如果您在连接 NS224 磁盘架的共享配置中使用 Cisco 9336C-FX2 交换机，并且需要升级 RCF 并为新的 ISL 端口应用新配置，请按照以下步骤操作：link:task_upgrade_rcf_files_on_mcc_ip_switches.html["升级 MetroCluster IP 交换机上的 RCF。"]
