...
[role="lead"]
您可能需要升级 Cisco IP 交换机上的参考配置文件 (RCF)。例如，ONTAP 升级或交换机固件升级都需要新的 RCF。
...
[cols="3*"]
|===
| 连接以下存储时 | 选择Cisco 9334c-打 的存储类型... | RCF 横幅/MOTD 示例 
...
* 此时将应用标准RCF配置。
* The https://mysupport.netapp.com/site/tools/tool-eula/rcffilegenerator["RcfFileGenerator"] 必须能够使用相同的版本和配置（平台、VLAN）创建所应用的相同 RCF。
* 所应用的 RCF 并非由 NetApp 为特殊配置提供。
* RCF 在应用前未更改。
* 在应用当前 RCF 之前，已按照步骤将交换机重置为出厂默认值。
* 应用RCF后、未对交换机(端口)配置进行任何更改。
+
如果不满足这些要求，则无法使用在生成 RCF 时创建的 CleanUpFiles。但是，您可以利用该函数来创建通用 CleanUpFiles——使用此方法的清理派生自 `show running-config`的输出，是最佳实践。
...
. 确定当前 RCF 版本以及使用的端口和 VLAN： `IP_switch_A_1# show banner motd`
+
...
| 2. | 11、12 | 259. | 259. 
|===
. [[Create-RCF-files-and-CleanUpFiles-or-create-generic-CleanUpFiles]] 为当前配置创建 RCF 和 CleanUpFiles，或创建通用 CleanUpFiles。
+
...
[role="tabbed-block"]
====
.选项 1：创建 RCF 并 CleanUpFiles
--
如果配置满足要求、请使用此操作步骤。

.步骤
.. 使用 RcfFileGenerator 1.4a（或更高版本）以及在步骤 1 中检索的信息创建 RCF。新版本的 RcfFileGenerator 会创建一组额外的 CleanUpFiles，可用于还原某些配置并准备交换机以应用新的 RCF 配置。
.. 将横幅 MOTD 与当前应用的 RCF 进行比较。平台类型、交换机类型、端口和 VLAN 使用情况必须相同。
+

NOTE: 您必须使用与 RCF 相同版本且完全相同配置的 CleanUpFiles。使用任何其他 CleanUpFile 将不起作用，并且可能需要完全重置交换机。

+

NOTE: 创建 RCF 所针对的 ONTAP 版本无关紧要。只有 RCF 版本才是重要的。

+

NOTE: RCF（即使是相同的版本）可能会列出更少或更多的平台。请确保已列出您的平台。
...
--
====
. [[Create-the-new-RCF-files-for-the-new-configuration]] 为新配置创建"新" RCF。创建这些文件的方式与上一步中创建文件的方式相同，但需选择相应的 ONTAP 和 RCF 版本。
+
完成此步骤后，您应该有两组 RCF，每组由十二个文件组成。

. 将文件下载到bootflash。
+
.. 下载您在 <<Create-RCF-files-and-CleanUpFiles-or-create-generic-CleanUpFiles,创建 RCF 和 CleanUpFiles，或为当前配置创建通用 CleanUpFiles>> 中创建的 CleanUpFiles
+

NOTE: 此 CleanUpFile 适用于当前应用的 RCF，而*不是*您要升级到的新 RCF。

+
Switch-A1的CleanUpFile示例：`Cleanup_NX9336_v1.81_Switch-A1.txt`

.. 下载您在 <<Create-the-new-RCF-files-for-the-new-configuration,为新配置创建"新" RCF。>> 中创建的"新" RCF
+
Switch-A1 的示例 RCF： `NX9336_v1.90_Switch-A1.txt`

.. 下载您在 <<Create-the-new-RCF-files-for-the-new-configuration,为新配置创建"新" RCF。>> 中创建的 CleanUpFiles。此步骤是可选的——您将来可以使用该文件更新交换机配置。它与当前应用的配置匹配。
+
...
+
如果发现已配置上述任何项目，则可能无法应用新的 RCF 配置。但是，您可以通过重新加载交换机*而不*将运行配置保存到启动配置来恢复到以前的配置。交换机将采用以前的配置。
...
cluster_A::*>
----
.. 将所有当前不在原端口上的集群 LIF 恢复到其原端口：
+
`network interface revert -vserver Cluster -lif *`

+
请稍等几分钟，以便 LIF 恢复到其主端口：
