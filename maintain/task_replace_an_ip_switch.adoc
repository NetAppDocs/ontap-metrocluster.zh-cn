...
CAUTION: 如果需要在 AFF A220、AFF A250 或 FAS2750 平台上执行交换机更换，请参阅 link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/Cluster_and_HA_communication_issues_during_back-end_switch_replacement["知识库文章"Cluster and HA communication issues during back-end switch replacement""^]。
...
[NOTE]
====
使用 X60134A 网络接口卡的平台不支持混合链路速度。要检查您的平台是否受到影响，请参见 https://hwu.netapp.com/["Hardware Universe"^]。

如果您要在受影响的平台上将使用 25G 端口的交换机替换为使用 100G 端口的新交换机（例如，在 AFF A50 平台上将 BES-53248 交换机替换为 Cisco 9336C 交换机），请参阅 link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/100G_link_not_coming_online_during_MetroCluster_back-end_switch_replacement?caseId=2010523490["知识库文章"MetroCluster 后端交换机更换期间 100G 链路未上线""^]。
...
NOTE: 您必须按以下顺序更新交换机：Switch_A_1、Switch_B_1、Switch_A_2、Switch_B_2。在第一个网络结构（Switch_A_1、Switch_B_1）中配置交换机后，必须重复这些步骤，以在第二个网络结构（Switch_A_2、Switch_B_2）上移动集群 LIF。请参阅 link:../install-ip/task_install_and_cable_the_mcc_components.html["了解有关配置 MetroCluster IP 交换机的信息"]。
...
. 为交换机布线。
+
您可以按照 link:../install-ip/using_rcf_generator.html["为 MetroCluster IP 交换机布线"] 中的步骤操作。
...
.. 如果本地集群接口*未*连接到交换机：
+
... 使用 link:https://docs.netapp.com/us-en/ontap-systems-switches/switch-bes-53248/migrate-to-2n-switched.html["迁移到交换式 NetApp 集群环境"^] 过程将无交换机集群转换为有交换机集群。使用 link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/using_rcf_generator.html["MetroCluster IP 安装和配置"] 中指示的端口或 RCF 布线文件连接本地集群接口。
...
. 打开交换机的电源。
+
如果新交换机相同，请接通新交换机的电源。如果要升级或降级交换机，请为两个交换机通电。在更新第二个网络结构之前，配置可以在每个站点使用两个不同的交换机运行。

. 重复执行、以验证MetroCluster 配置是否运行正常 <<all_step1,第 1 步>>。
+
如果您正在升级或降级第一个网络结构中的交换机，您可能会看到一些与本地集群相关的警报。

+

NOTE: 如果要升级或降级网络结构，请对第二个网络结构重复所有步骤。
