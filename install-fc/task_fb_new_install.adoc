...
可以从 ONTAP 界面运行网桥命令行界面命令，例如 `storage bridge run-cli -namebridge-name-commandbridge-command-name` 命令。
...
. 如果需要，可发出 `set closePort _service_` 并根据需要重复执行命令，直到禁用所有所需服务为止，以禁用服务：
+
`set closePort _service_`

+
`set closePort` 命令一次禁用一项服务。可以通过以下方式之一指定此服务：
...
. 保护对网桥的访问并保存网桥的配置。根据您的系统运行的 ONTAP 版本，从下方选择一个选项。
+
[cols="1,3"]
|===
