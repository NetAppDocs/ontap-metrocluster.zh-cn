...
[NOTE]
====
配置 FibreBridge 7600N 网桥时，不需要 FCConnMode 设置。如果使用的是命令行界面，则必须运行以下命令：
...
.步骤
. 【第 1 步 _verify_bridge]] 验证每个网桥是否可以检测到它所连接的所有磁盘驱动器和磁盘架：
+
[cols="1,3"]
|===
| 如果您使用的是 ... | 那么 ... 
...
.. 单击 * 高级 * 。
.. 使用 `sasargets` 命令查看已连接的设备，然后单击 * 提交 * 。
...
正确
 a| 
重复 <<step1_verify_bridges,第 1 步>> 其余每个网桥。
...
+
link:task_fb_new_install.html["使用缆线将磁盘架连接到网桥"]

.. 重复 <<step1_verify_bridges,第 1 步>>。
