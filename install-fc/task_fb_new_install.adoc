...
可以从 ONTAP 界面上的 ONTAP interface storage bridge run-cli -namebridge-name-commandbridge-command-name 命令发出网桥命令行界面命令。
...
NOTE: 支持带内 ACP ，而无需在以下磁盘架和 FibreBridge 7500N 或 7600N 网桥中进行额外布线：
...
. 使用控制台启用访问服务，然后保存配置： `set closePort none``saveconfiguration`
+
set closePort none 命令可启用网桥上的所有访问服务。

. 如果需要，可通过发出 set closePort 并根据需要重复执行命令，直到禁用所有所需服务： `set closePort service` 来禁用服务
+
set closePort 命令一次禁用单个服务。
...
. 如果要启用 SNMP ，还必须对 set snmp enabled 命令执行问题描述操作： `set snmp enabled`
+
SNMP 是唯一需要单独的 enable 命令的协议。

. 保存配置： `saveconfiguration`
...
+
如果使用命令行界面，则必须运行以下命令：``set ipaddress MP1 IP-address```set ipsubnetmask MP1 subnet-mask``set ipgateway MP1 x.x.x.x``set ipdhcp MP1 disabled``set ethernetspeed MP1 1000`
...
*** bridge_A_1b
*** bridge_B_1a
*** bridge_B_1b 如果使用命令行界面，则必须运行以下命令：``set bridgename bridgename``
...
*** FibreBridge 6500 网桥最多支持 8 ， 4 或 2 Gbps 。* 注： * 您选择的 FCDataRate 速度仅限于网桥和网桥端口所连接的控制器模块的 FC 端口所支持的最大速度。布线距离不得超过 SFP 和其他硬件的限制。




+
如果使用命令行界面，则必须运行以下命令： `set FCDataRate port-numberport-speed`
...
+
如果使用命令行界面，则必须运行以下命令： `set FCConnMode port-number ptp`
...
.. 如果启用了以太网端口 MP1 ，请运行： `set EthernetPort MP1 disabled`
+
...
|===
. 完成 MetroCluster 配置后，使用 flashimages 命令检查您的 FibreBridge 固件版本，如果网桥未使用最新支持的版本，请更新配置中所有网桥上的固件。
+
...
xref:task_fb_new_install.adoc[FC-SAS 网桥的带内管理]
...
+
+ 图像： ./media/mcc_bridge_bridge_and_sas_stack_and_7500n_and_single _stack.gif]]
...
. 验证每个网桥是否都可以检测到它所连接的所有磁盘驱动器和磁盘架：
+
|===
...
NOTE: 如果输出开头显示文本 response truncated ，则可以使用 Telnet 连接到网桥并输入相同的命令来查看所有输出。
...
正确
 a| 
重复 xref:task_fb_new_install.adocSTEP_524EBC3334F54467B771D9E2FD2B2DDA[第 1 步] 其余每个网桥。
...
+
xref:task_fb_new_install.adoc[使用缆线将磁盘架连接到网桥]

.. 重复 xref:task_fb_new_install.adocSTEP_524EBC3334F54467B771D9E2FD2B2DDA[第 1 步]。
...
----
. 从包含网桥的集群的 ONTAP 提示符处，保存网桥配置： `storage bridge run-cli -bridge bridge-name -command saveconfiguration`
+
以下命令可保护 bridge_A_1 的安全：
...
----
. 从包含网桥的集群的 ONTAP 提示符处，重新启动网桥的固件： `storage bridge run-cli -bridge bridge-name -command firmwarerestart`
+
以下命令可保护 bridge_A_1 的安全：
