...
节点 1 IC LIF 2
 ...
 a| 
双节点 MetroCluster 配置（每个站点一个节点）不需要节点 2 IC LIF 1 。
 a| 
 a| 
 a| 
 a| 
...
+
.. 在第一个节点上，系统地将本地磁盘架分配给池 0 ，并将远程磁盘架分配给池 1 ： `disk assign -shelf local-switch-name ： shelf-name.port -p pool`
+
...
----
.. 对本地站点的第二个节点重复此过程，系统地将本地磁盘架分配给池 0 ，并将远程磁盘架分配给池 1 ： `disk assign -shelf local-switch-name ： shelf-name.port -p pool`
+
...
+
.. 在远程站点的第一个节点上，系统地将其本地磁盘架分配给池 0 ，并将其远程磁盘架分配给池 1 ： `disk assign -shelf local-switch-namelf-name -p pool`
+
...
----
.. 对远程站点的第二个节点重复此过程，系统地将其本地磁盘架分配给池 0 ，并将其远程磁盘架分配给池 1 ： `disk assign -shelf shelf-name -p pool`
+
...
+
.. 在第一个节点上，系统地将每个磁盘架上一半的磁盘分配给池 0 ，而将另一半磁盘分配给 HA 配对节点的池 0 ： `disk assign -disk disk-name -p pool -n number-of-disks`
+
...
----
.. 对本地站点的第二个节点重复此过程，系统地将每个磁盘架上一半的磁盘分配给池 1 ，而将另一半磁盘分配给 HA 配对节点的池 1 ： `disk assign -disk disk-name -p pool`
+
如果存储控制器 Controller_A_1 有四个磁盘架，每个磁盘架具有 8 个 SSD ，则您可以问题描述执行以下命令：
...
+
.. 在远程站点的第一个节点上，系统地将每个磁盘架上一半的磁盘分配给池 0 ，而将另一半磁盘分配给 HA 配对节点的池 0 ： `disk assign -disk disk-name -p pool`
+
...
----
.. 对远程站点的第二个节点重复此过程，系统地将每个磁盘架上一半的磁盘分配给池 1 ，而将另一半磁盘分配给 HA 配对节点的池 1 ： `disk assign -disk disk-name -p pool`
+
...
. 如果您的系统是从工厂收到的，请确认磁盘架分配： `disk show – v`
...
+
.. 如果尚未启动，请将每个系统启动至维护模式。
.. 在站点 A 的节点上，系统地将本地磁盘架分配给池 0 ，并将远程磁盘架分配给池 1 ： `disk assign -shelf disk_shelf_name -p pool`
+
...
----
.. 在远程站点（站点 B ）的节点上，系统地将其本地磁盘架分配给池 0 ，并将其远程磁盘架分配给池 1 ： `disk assign -shelf disk_shelf_name -p pool`
+
...
. 在维护模式下，显示控制器模块和机箱的 HA 状态： `ha-config show`
+
...
. 完成集群设置向导并退出后，请验证集群是否处于活动状态且第一个节点运行状况良好： `cluster show`
+
...
+
如果系统处于维护模式，请使用问题描述 halt 命令退出维护模式，然后问题描述从加载程序提示符处运行以下命令： `boot_ontap`
...
+
否则，您必须在每个节点上执行问题描述以下命令并重新启动节点： `storage failover modify -mode ha -node localhost`
...
xref:concept_prepare_for_the_mcc_installation.adoc[使用专用端口时的注意事项]

xref:concept_prepare_for_the_mcc_installation.adoc[共享数据端口时的注意事项]
...
+
.. 在数据保护目标集群上，创建与数据保护源集群的对等关系：``cluster peer create -peer-addrs peer_LIF_ips -ipspace ipspace``
+
如果您不使用自定义 IP 空间，则可以忽略 -ipspace 选项。有关完整的命令语法，请参见手册页。
...
.. 在数据保护源集群上，向目标集群验证源集群的身份：``cluster peer create -peer-addrs peer_LIF_ips -IPspace IPspace``
+
有关完整的命令语法，请参见手册页。
...
|===
+
* 注： * 最佳实践是使用多个数据聚合。如果第一个 DR 组只有一个聚合，而您要添加一个具有一个聚合的 DR 组，则必须将元数据卷从单个数据聚合中移出。有关此操作步骤的详细信息，请参见 http://docs.netapp.com/ontap-9/topic/com.netapp.doc.hw-metrocluster-service/GUID-114DAE6E-F105-4908-ABB1-CE1D7B5C7048.html["在 MetroCluster 配置中移动元数据卷"]。
...
+
... 进入高级模式： `set advanced`
... 验证设置： `MetroCluster interconnect adapter show`
+
[listing]
----
...
+
... 进入高级模式： `set advanced`
... 验证 MetroCluster 配置设置： `MetroCluster interconnect adapter show`
+
[listing]
----
mcc4-b12_siteB::*> metrocluster interconnect adapter show
...
8 entries were displayed.
----
... 在节点 `mCC4-B1` 和节点 `mCC4-B2` 上启用 OOD ： `MetroCluster 互连适配器修改 -node node name -is-ood-enabled true`
+
[listing]
----
...
. 在控制器提示符处为每个交换机创建一个 SNMP 用户： `ssecurity login create`
+
[listing]
----
...
10.10.11
----
. 检查每个交换机是否有一个登录条目： `ssecurity login show`
+
[listing]
----
...
----
. 从交换机提示符处为交换机配置 SNMPv3 ： `snmpconfig -set SNMPv3`
+
...
. 向每个 MetroCluster 节点添加一个具有 IP 地址的交换机： `storage switch add -address ipaddress`
+
...
----
. 验证是否已正确配置所有交换机： `storage switch show`
+
...
+
.... 确认已启用存储故障转移： `storage failover show`
+
输出应指示两个节点均可进行接管：
...
----
.... 从 controller_A_1 接管 controller_A_2 ： `storage failover takeover controller_A_2`
+
您可以使用 storage failover show-takeover 命令监控接管操作的进度。

.... 确认接管已完成： `storage failover show`
+
...
2 entries were displayed.
----
.... 交还 controller_A_2 ： `storage failover giveback controller_A_2`
+
您可以使用 storage failover show-giveback 命令监控交还操作的进度。

.... 确认存储故障转移已恢复正常状态： `storage failover show`
+
输出应指示两个节点均可进行接管：
