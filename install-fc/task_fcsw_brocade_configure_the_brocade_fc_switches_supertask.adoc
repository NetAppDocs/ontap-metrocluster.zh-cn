...
+
xref:concept_tdm_wdm.adoc[在光纤连接 MetroCluster 配置中使用 TDM/WDM 设备的注意事项]
...
. 持久禁用交换机： `sswitchcfgpersistentdisable`
+
...
BrocadeSwitchA:admin> switchcfgpersistentdisable
----
. 输入 `sswitchname switch_name` 设置交换机名称。
+
...
FC_switch_B_1:admin>
----
. 对每个端口发出以下命令，将所有端口设置为其默认值： `portcfgdefault`
+
必须对交换机上的所有端口执行此操作。
...
----
. 发出以下命令以清除分区信息： `cfgdisable``cfgclear``cfgsave`
+
以下示例显示了对 FC_switch_A_1 执行的命令：
...
----
. 将常规交换机设置设置为 default ： `configdefault`
+
以下示例显示了对 FC_switch_A_1 执行的命令：
...
----
. 将所有端口设置为非中继模式： `sswitchcfgtrunk 0`
+
以下示例显示了对 FC_switch_A_1 执行的命令：
...
----
. 在 Brocade 6510 交换机上，禁用 Brocade 虚拟网络结构（ VF ）功能： `fosconfig options`
+
以下示例显示了对 FC_switch_A_1 执行的命令：
...
... 使用 `iodshow` ， `aptpolicy` 和 `dlsshow` 命令验证 OOD 设置。
+
例如，对 FC_switch_A_1 执行问题描述命令：
...
. * 可选 * ：如果将社区字符串设置为 "`公有` " 以外的值，则必须使用指定的社区字符串配置 ONTAP 运行状况监控器：
+
.. 更改现有社区字符串：
+
...
xref:concept_prepare_for_the_mcc_installation.adoc[使用 Brocade DCX 8510-8 交换机的要求]
...
|===
.步骤
. 【第 1-Brocade-config ，第 1 步 ]] 配置端口速度：
+
...
3: fffc03 10:00:00:05:33:8c:2e:9a 10.10.10.65  0.0.0.0    >"FC_switch_B_1
----
. 【步骤 10-Brocade-config ，步骤 10]] 确认 ISL 的中继：
+
`Trunkshow`
...
. 重复 <<step1-brocade-config>> 到 <<step10-brocade-config>> 用于第二个 FC 交换机网络结构。
...
xref:task_fcsw_brocade_configure_the_brocade_fc_switches_supertask.adoc[FC-VI 端口的分区]

xref:task_fcsw_brocade_configure_the_brocade_fc_switches_supertask.adoc[使用一个 FC 端口为 FibreBridge 6500N 网桥， FibreBridge 7500N 或 7600N 网桥分区]

xref:task_fcsw_brocade_configure_the_brocade_fc_switches_supertask.adoc[使用两个 FC 端口的 FibreBridge 7500N 网桥的分区]
...
+
`portCfgEncrypt — enable 8``portCfgEncrypt —启用 12`
