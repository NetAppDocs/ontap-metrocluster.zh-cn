...
NOTE: 在配置聚合之前，必须按照中的 " 在 MetroCluster 配置中为 FabricPool 设置对象存储 " 中所述设置对象存储https://["磁盘和聚合管理"]
...
. 创建包含聚合的 FabricPool 镜像，如中所述 https://["磁盘和聚合管理"]
+
.. 附加主对象存储。
+
此对象存储在物理上更接近集群。
...
从ONTAP 9.11.1开始、 https://["一致性组"^] 在MetroCluster 配置中受支持。
