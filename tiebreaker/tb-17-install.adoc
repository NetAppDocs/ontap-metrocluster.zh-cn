...
* 你只能从 Tiebreaker 1.6P1 升级到 Tiebreaker 1.7。参考link:tb-16-install.html["安装 Tiebreaker 1.6 或 1.6P1"]。
* 您可以使用具有足够管理权限的非root用户身份安装MetroCluster Tieb破碎 镐、以便执行Tieb破碎 镐安装、创建表和用户以及设置用户密码。



NOTE: 安装 Tiebreaker 1.7 后，您需要link:concept_configuring_the_tiebreaker_software.html#import-certificates-tb17["导入证书"]以启用无缝监控。

.步骤
. 下载MetroCluster Tiebreaker 1.7 软件。
+
...
. 以 root 用户身份登录到主机。
. 如果您正在升级，请检查您正在运行的 Tiebreaker 版本：
+
以下示例展示了决胜规则 1.6P1
...
NOTE: 如果由于缺少证书而导致升级失败并出现错误，请参阅以下内容：link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#import-certificates-tb17["进口证书"] 。证书导入后，您可以再次尝试升级。
