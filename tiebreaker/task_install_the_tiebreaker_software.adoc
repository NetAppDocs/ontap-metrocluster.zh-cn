...
.步骤
. 下载 "NetApp-MetroCluster-Tiebreaker 软件 -1.3-1.x86_64 ： rpm" 文件。
+
...
执行新安装
 a| 
`rpm -ivh netapp-metrocluster-tiebreaker 软件 -1.3-1x86_64 ： rpm`
...
升级现有安装
 a| 
`rpm -Uvh NetApp-MetroCluster-Tiebreaker 软件 -1.3-1.x86_64 ： rpm`
...
如果您在升级之前将 Tiebreaker 监控器置于观察模式，则可以升级运行该监控器的主机 o ，而不会造成任何中断。
