...
image::../media/mcc_upgrade_cluster_a_in_switchover.png[MetroCluster切换后，Site_B 处于非活动状态并准备升级]
...
. 如果使用根卷加密，请根据您使用的 ONTAP 版本，选择引导菜单选项或为密钥管理配置再次发出引导菜单命令。
+
[role="tabbed-block"]
====
.ONTAP 9.8及更高版本
...
image::../media/mcc_upgrade_cluster_a_switchback.png[MetroCluster切换后，Site_A 等待升级]
