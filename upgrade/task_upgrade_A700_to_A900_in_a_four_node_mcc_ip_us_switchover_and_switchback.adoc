...
. 如果 MetroCluster 节点配置了管理 SVM（集群）的外部密钥管理器，请将密钥服务器信息复制到安全位置。如果已配置板载密钥管理器，请将备份信息和密码复制到安全位置。
+
[role="tabbed-block"]
====
.板载密钥管理器
--
.. 运行以下命令来显示 Onboard Key Manager 备份：
+
`s安全密钥管理器板载 show-backup`

+
稍后在升级过程中，您还需要最新的板载密钥管理器密码。
...
cluster_B
            admin   -          -          -           -          -
----
.. 获取与 admin SVM（集群）上配置的外部密钥管理器关联的主密钥服务器的地址和端口号：
+
[NOTE]
====
*** 管理 SVM 最多可以有四个主密钥服务器，但您只需要其中一个主密钥服务器的端口号和地址，即可在后面的过程中还原密钥管理配置。
*** 如果与所选主密钥服务器关联有用户名和密码，则还需要该信息用于恢复过程。
...
----
.. 如果运行的是 ONTAP 9.16.1 或更高版本，请获取管理 SVM 的密钥库 UUID。
+
...
cluster_B::*
----
.. 将您收集的信息保存在安全的位置，以便稍后在程序中使用。您现在应该已经收集了以下信息：
+
*** 管理 SVM 的一个主密钥服务器的端口号和地址
*** 用户名和密码（如果与所选主密钥服务器关联）
*** 管理 SVM 的密钥库 UUID（ONTAP 9.16.1 或更高版本中必需）
...
. 如果 MetroCluster 系统的管理 SVM 配置了密钥管理器，请为您的密钥管理配置选择正确的启动菜单选项。
+
|===
...
*注意：*虽然管理 SVM 最多可以有四个主密钥服务器，但在恢复过程中只能指定一个主密钥服务器的信息。
...
NOTE: 运行启动菜单选项 `6`时，可能需要重置 MetroCluster bootarg 值。请参阅link:task_upgrade_A700_to_A900_in_a_four_node_mcc_ip_us_switchover_and_switchback.html#set-the-metrocluster-ip-bootarg-variables["设置 MetroCluster IP bootarg 变量"]。
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["正在添加 MetroCluster 配置"] 在_MetroCluster Tiebreaker 安装和配置_部分。
