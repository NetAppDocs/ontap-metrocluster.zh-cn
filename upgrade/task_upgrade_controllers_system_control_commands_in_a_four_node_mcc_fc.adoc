...
* 可以使用此过程，通过基于 NSO 的自动切换和切换回来升级四节点 MetroCluster FC 配置中的控制器。如果要使用聚合重定位 (ARL) 执行控制器升级，请参阅 link:https://docs.netapp.com/us-en/ontap-systems-upgrade/upgrade-arl-auto-app/["使用 system controller replace 命令升级运行 ONTAP 9.8 或更高版本的控制器硬件"^]。建议使用基于 NSO 的自动化程序。
