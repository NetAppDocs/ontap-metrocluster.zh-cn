...
* 有关支持的平台升级组合的信息、请查看中的MetroCluster FC升级表 link:concept_choosing_controller_upgrade_mcc.html#metrocluster-fc-controller-upgrades["选择控制器升级操作步骤"]。


请参见 https://["选择升级或刷新方法"] 以了解其他过程。
...
* 您可以使用此操作步骤通过基于 NSO 的自动切换和切回升级四节点 MetroCluster FC 配置中的控制器。如果要使用聚合重新定位（ Aggregate Relocation ， ARL ）执行控制器升级，请参见 https://["使用 system controller replace 命令升级运行 ONTAP 9.8 或更高版本的控制器硬件"]。建议使用基于 NSO 的自动化操作步骤。
...
.步骤
. http://["删除现有 MetroCluster 配置"^] 来自 Tiebreaker 软件。
...
+
有关平台型号的端口使用情况，请参见 https://["NetApp Hardware Universe"^]
...
.步骤
. 访问 https://["NetApp 支持站点"^] 下载用于执行系统网络启动的文件。
...
. 使用中的步骤 http://["正在添加 MetroCluster 配置"]。
