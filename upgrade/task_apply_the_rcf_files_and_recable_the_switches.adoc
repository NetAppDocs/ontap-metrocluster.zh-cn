...
[role="lead"]
您必须应用参考配置文件 (RCF) 以重新配置分区，从而适应新节点。

如果无法使用 RCF 配置交换机，则必须手动配置交换机。请参阅：
...
. 找到适用于您的配置的 RCF。
+
对于与交换机型号匹配的八节点配置，必须使用 RCF。

. 应用 RCF，按照下载页面上的说明，根据需要调整 ISL 设置。
. 确保已保存交换机配置。
. 重新启动 FC 交换机。
. 使用先前创建的布线布局，使用缆线将原有的和新的 FC-SAS 网桥连接到 FC 交换机。
+
FC 交换机端口使用情况必须与 link:../install-fc/index.html["光纤连接的 MetroCluster 安装和配置"] 中所述的 MetroCluster 八节点使用情况相匹配，才能使用 RCF。
