...
NOTE: 在过渡和系统刷新工作流程中使用这些步骤。您可能已经完成了此流程中的一些步骤，这些步骤是这些工作流程的一部分。
...
NOTE: 只有 ONTAP 9.8 及更高版本才支持 `MetroCluster remove-dr-group` 命令。
...
+
在 MetroCluster FC 配置中，转到 <<fc_step_7_remove_nodes,删除旧的 DR 组节点>> 的步骤。
...
.. 删除要删除的每个 DR 组节点上的 pool1 磁盘的所有权。在要删除的每个节点上执行这些步骤。在此步骤中使用的示例中，将删除 node_A_1。
+
... 转到 nodeshell：
+
`run -node <node_name>`
...
+
将显示所有磁盘，包括节点拥有的 Pool1 磁盘。以下示例显示了磁盘 0n.4 及其分区分配给 Pool1 的截断输出。输出将根据您的配置而有所不同。
...
... 删除每个 pool1 磁盘的磁盘所有权：
+
如果配置包含分区磁盘，则首先删除每个分区磁盘的所有权，然后删除容器磁盘的所有权：

+
`disk remove_ownership <disk_name>`

+
在以下示例中，在删除容器磁盘 `0n.4`的所有权之前，将删除分区磁盘的所有权：
...
. [[fc_step_7_remove_nodes]]删除旧 DR 组中的节点。
+
在每个集群上执行此步骤。
...
.相关信息
在link:https://docs.netapp.com/us-en/ontap-cli/["ONTAP 命令参考"^]中详细了解此过程中使用的命令。
