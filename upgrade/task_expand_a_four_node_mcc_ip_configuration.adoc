...
* 下表显示了使用*共享 MetroCluster/HA 端口*和*共享集群/HA 端口*的所有平台型号。并非所有组合都受此程序支持。请参阅<<expand-ip-supported-platforms,支持的平台组合>>以验证您的扩展组合是否受支持。
...
下表显示了八节点 MetroCluster IP 配置支持的平台组合。
...
image:../media/expand-ip-group-1-updated.png["MetroCluster IP 中AFF和FAS组 1 平台的扩展组合"]
...
image:../media/expand-ip-group-2-updated.png["MetroCluster IP 中AFF和FAS组 2 平台的扩展组合"]
...
. 在维护模式下，发出 halt 命令以退出维护模式：
+
`halt`

. 检查 partner-sysid 是否正确：
+
...
+
有关完整的命令语法，请参见 link:https://docs.netapp.com/us-en/ontap-cli["ONTAP 命令参考"^]。
...
+
以下示例将端口 "e0e" 和 "e0f" 分配给 SVM "cluster01" 上的故障转移组 "intercluster01"：
...
+
以下示例显示端口 "e0e" 上的集群间 LIF "cluster01_icl01" 和 "cluster01_icl02" 将故障转移到端口 "e0f"。
...
== 创建 MetroCluster 接口并镜像根聚合

必须在新 MetroCluster IP 节点上创建 MetroCluster 接口并镜像根聚合。
...
.步骤
. 在每个站点上，将新的集群间 LIF 添加到对等关系中。
+
从 cluster_A 中，运行以下命令：
...
NOTE: 以下命令的输出可能会因平台型号而异： `metrocluster configuration-settings connection show`例如，在某些平台上，HA Partner 的行不会显示在输出中。
...
[NOTE]
====
** 每个站点必须至少创建一个镜像数据聚合。建议在 MetroCluster IP 节点上为每个站点配置两个镜像数据聚合以托管 MDV 卷，但支持每个站点一个聚合（但不建议这样做）。MetroCluster的一个站点具有一个镜像数据聚合、而另一个站点具有多个镜像数据聚合、这是可以接受的。
