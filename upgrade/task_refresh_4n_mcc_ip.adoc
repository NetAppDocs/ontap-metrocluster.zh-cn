...
* 下表显示了使用*共享 MetroCluster/HA 端口*和*共享集群/HA 端口*的所有平台型号。并非所有组合都受此程序支持。请参阅link:concept_choosing_tech_refresh_mcc.html#supported-metrocluster-ip-tech-refresh-combinations["支持的平台组合"]以验证您的刷新组合是否受支持。
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["正在添加 MetroCluster 配置"] 在_MetroCluster决胜规则安装和配置_中。
