...
此操作步骤支持在运行ONTAP 9.13.1及更高版本的系统上进行以下控制器升级。
...
NOTE: 您可以使用所述的手动升级操作步骤在运行ONTAP 9.12.1或9.11.1的系统上执行上表中列出的控制器升级 link:task_upgrade_controllers_in_a_four_node_ip_mcc_us_switchover_and_switchback_mcc_ip.html["此处"]。
...
* 如果在系统上启用了该功能、 link:../maintain/task-configure-encryption.html#disable-end-to-end-encryption["禁用端到端加密"] 在执行升级之前。
