...
NOTE: 使用 BES-53248 IP 交换机时，不支持升级 AFF A320 平台型号。
...
+
按照中适用于您的交换机供应商的一节中的步骤进行操作 link:link:../install-ip/index.htmll["《 MetroCluster IP 安装和配置指南》"]。
...
. 下载并安装 RCF 文件。
+
按照中的步骤进行操作 link:link:../install-ip/index.htmll["《 MetroCluster IP 安装和配置指南》"]。
...
FAS/AFF8000 系列系统
 a| 
将 ` _ontap-version____image.tgz` 文件的内容提取到目标目录： `tar -zxvf _ontap-version___image.tgz`


NOTE: 如果要在 Windows 上提取内容，请使用 7-Zip 或 WinRAR 提取网络启动映像。您的目录列表应包含一个包含内核文件 netboot/kernel 的网络启动文件夹
...
` _ontap-version___image.tgz`

您不需要提取 ` _ontap-version___image.tgz` 文件。
...
` * ifconfig e0M -addr=ip_addr -mask=_netmask__*`
...
所有其他系统
 a| 
` * 网络启动 http://__web_server_ip/path_to_web-accessible_directory/ontap-version___image.tgz*`[]

|===
.. 从启动菜单中，选择选项 10\（ 7\ ） Install new software first* 以将新软件映像下载并安装到启动设备。
...
. 如果系统提示您继续运行操作步骤，请输入 ` * y*` ，当系统提示您输入软件包时，请输入映像文件的 URL ：
+
...
.. 验证连接到节点的设备： + ` * network device-discovery show*`
+
您应在每个集群上问题描述此命令。
...
+
.. 确认 MetroCluster 配置以及操作模式为 `normal` ： + ` * MetroCluster show*`
...
* 查看您在中收集的信息 xref:task_upgrade_controllers_in_a_four_node_ip_mcc_us_switchover_and_switchback_mcc_ip.adoc[将端口从旧节点映射到新节点]。
...
.. 修改所有数据 LIF 以使用通用端口作为主端口： + ` * 网络接口 modify -vserver _svm-name_ -lif _data-lif_ -home-port _port-id_*`
+
在我们的示例中，此值为 e0d 。
...
+
link:../install-ip/index.htmll["MetroCluster IP 安装和配置"]
...
. 下载并安装 RCF 文件。
+
按照中适用于您的交换机供应商的一节中的步骤进行操作 link:../install-ip/index.htmll["MetroCluster IP 安装和配置"]。
...
在此任务中，您将使用先前在中的升级操作步骤中标识的 UUID 和系统 ID xref:task_upgrade_controllers_in_a_four_node_ip_mcc_us_switchover_and_switchback_mcc_ip.adoc[升级前收集信息]。
...
+
` * setenv bootarg.mcc.aux_partner_uuid _DR-aux-partner-node-UUUID_*`

+
` * 设置 v bootarg.mcc_iscsi.node_uuid _local-node-UUUUID_*` ..设置 node_B_1-new 上的 UUID 。
...
+
` * setenv bootarg.mcc.adb_enabled true*`
...
+
` * 设置 v bootarg.kmip.init.ipaddr*`

+
` * 设置 v bootarg.kmip.kmip.init.netmask*`

+
` * 设置 v bootarg.kmip.kmip.init.gateway*`

+
` * 设置 v bootarg.kmip.kmip.init.interface*`
...
+
使用中的步骤 xref:task_upgrade_controllers_in_a_four_node_ip_mcc_us_switchover_and_switchback_mcc_ip.adoc[设置 MetroCluster IP bootarg 变量]。
...
|===
+
您可能需要在启动菜单提示符处多次问题描述 `re封装 _xxxxxxxx_keymanager` 命令和选项 6 ，直到节点完全启动为止。
...
* 此任务在 site_B 上执行
* 请参见您在中创建的端口映射计划 xref:task_upgrade_controllers_in_a_four_node_ip_mcc_us_switchover_and_switchback_mcc_ip.adoc[将端口从旧节点映射到新节点]。
...
+
.. 修复数据聚合： + ` * MetroCluster heal aggreges*`
.. 修复根聚合：
+
...
.. 检查是否已启用本地故障转移： `storage failover show`
...
.步骤
. 从开始，重复上述步骤升级 cluster_A 上的节点 xref:task_upgrade_controllers_in_a_four_node_ip_mcc_us_switchover_and_switchback_mcc_ip.adoc[正在准备升级]。
+
