...
.步骤
. 将 MetroCluster 配置切换到 Site_A ，以便可以升级 Site_B 的节点：
+
.. 在 Site_A 上执行问题描述以下命令：
+
...
.步骤
. 连接到站点 B 上旧控制器（ node_B_1-700 和 node_B_2-700 ）的串行控制台，并验证其是否显示 `LOADER` 提示符。
. 从站点 B 的两个节点收集 bootarg 值： `printenv`
. 关闭站点 B 的机箱




== 从站点 B 的两个节点上卸下控制器模块和 NVS
...
使用以下操作步骤在站点 B 的两个节点的插槽 6 中安装 AFF A900 NVS
...
image::../media/mcc_upgrade_cluster_a_switchback.png[四节点 MetroCluster]
