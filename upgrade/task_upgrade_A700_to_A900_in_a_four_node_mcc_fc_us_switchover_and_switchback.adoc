...
. 重复更新网络结构 B 交换机（交换机 2 和 4 ） ,第 2 步 to ,第 5 步。
...
|===
. 在节点 1 上执行 `netboot` ： `netboot http://<web_server_ip/path_to_web_accessible_directory>/netboot/kernel`[]` <path_to_the_web-accessible_directory>` 应指向您在中下载 ` <ontap_version>_image.tgz` 的位置 ,第 2 步。
+
