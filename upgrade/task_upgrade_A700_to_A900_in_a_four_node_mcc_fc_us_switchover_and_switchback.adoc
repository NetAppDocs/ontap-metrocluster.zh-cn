...
image::../media/mcc_upgrade_cluster_a_in_switchover_A900.png[Site_B 处于非活动状态并准备升级]
...
. 如有必要，请以 UTC 或格林威治标准时间（GMT）设置日期：
+
`set date <mm/dd/yyyy>`
...
. 请按照以下步骤操作： link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["正在添加 MetroCluster 配置"] 在_MetroCluster Tiebreaker 安装和配置_部分。
