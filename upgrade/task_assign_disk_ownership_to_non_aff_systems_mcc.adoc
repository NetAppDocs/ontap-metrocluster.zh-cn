...
+
.. 在第一个节点上，系统地将本地磁盘架分配给池 0 ，并将远程磁盘架分配给池 1 ： + ` * disk assign -shelf _local-switch-name ： shelf-name.port_ -p _pool_*`
+
...
----
.. 对本地站点的第二个节点重复此过程，系统地将本地磁盘架分配给池 0 ，并将远程磁盘架分配给池 1 ： + ` * disk assign -shelf _local-switch-name ： shelf-name.port_ -p _pool_*`
+
...
+
.. 在远程站点的第一个节点上，系统地将其本地磁盘架分配给池 0 ，并将其远程磁盘架分配给池 1 ： + ` * disk assign -shelf _local-switch-namelf-name_ -p _pool_*`
+
...
----
.. 对远程站点的第二个节点重复此过程，系统地将其本地磁盘架分配给池 0 ，并将其远程磁盘架分配给池 1 ： + ` * disk assign -shelf _shelf-name_ -p _pool_*`
+
