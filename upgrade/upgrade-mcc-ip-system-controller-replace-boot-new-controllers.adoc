...
+
请按照 link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["设置 MetroCluster IP bootarg 变量"] 中的步骤操作。

. 验证 partner-sysid 是否正确：
+
`printenv partner-sysid`
...
NOTE: 运行启动菜单选项 `6`时，可能需要重置 MetroCluster bootarg 值。请参阅link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["设置 MetroCluster IP bootarg 变量"]。
...
.. 集群间 LIF 启动后，请检查集群对等状态，并根据需要重新建立集群对等。
+
您可能需要重新配置集群对等关系。
