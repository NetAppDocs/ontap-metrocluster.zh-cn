...
NOTE: 从 ONTAP 9.8 开始， ` * storage bridge*` 命令将替换为 ` * system bridge*` 。以下步骤显示了 ` * storage bridge*` 命令。但是，如果您运行的是 ONTAP 9.8 或更高版本，则首选使用 ` * system bridge*` 命令。
...
+
` * 设置 FCDataRate _port-numberport-speed _ *`
...
*** 如果不使用第二个端口，则必须禁用端口： + ` * FCPortDisable port-number*` 以下示例显示了禁用 FC 端口 2 ：
...
.. 如果启用了以太网端口 MP1 ，请运行： ` * 设置 EthernetPort MP1 disabled" *`
+
