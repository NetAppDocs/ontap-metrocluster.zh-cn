...
* 当您对连接 NS224 存储的 Cisco 9336C-FX2 交换机进行布线时，最多只能有两个 MetroCluster 配置或 DR 组。必须至少有一个 MetroCluster 配置或 DR 组将 NS224 盘架连接到该 MetroCluster 交换机。如果其中一个 MetroCluster 配置或 DR 组是不支持 NS224 盘架的系统，则它只能作为第二个 MetroCluster 配置或 DR 组连接。
+
