...
CAUTION: 从ONTAP 9.12.1开始、您可以在MetroCluster IP配置中启用MetroCluster 自动强制切换功能。此功能是调解器辅助计划外切换的扩展。启用此功能之前、请查看 link:concept-risks-limitations-automatic-switchover.html["使用MetroCluster 自动强制切换的风险和限制"]。
...
. 如果要配置MetroCluster 自动强制切换、请运行以下命令： `metrocluster modify -allow-automatic-forced-switchover true`
+
启用此功能之前、请查看 link:concept-risks-limitations-automatic-switchover.html["使用MetroCluster 自动强制切换的风险和限制"]。
