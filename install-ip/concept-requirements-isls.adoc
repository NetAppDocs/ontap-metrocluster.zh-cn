...
以下要求适用于所有 MetroCluster IP 配置上的 ISL：
...
* 所需的最低 ISL 速度取决于平台型号：
+
** 从 ONTAP 9.18.1 开始，MetroCluster IP 后端端口速度为 100Gbps 的平台要求最低 ISL 链路速度为 100Gbps。使用不同的 ISL 速度需要功能策略差异请求 (FPVR)。要提交 FPVR，请联系您的 NetApp 销售团队。
+

CAUTION: 在当前不符合 100Gbps ISL 要求的平台上升级到 ONTAP 9.18.1 及更高版本不会被阻止，可以继续进行。但是，NetApp 强烈建议客户过渡到 100Gbps ISL 连接，以保持预期的性能和可用性水平。
