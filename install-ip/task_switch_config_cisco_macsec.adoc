...
summary: 如果需要，仅在站点之间运行的 WAN ISL 端口上配置 MACsec 加密。如果选择使用 MACsec 加密，请在应用正确的 RCF 文件后进行配置。 
---
= 在 MetroCluster IP 站点的 Cisco 9336C 交换机上配置 MACsec 加密（可选）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果需要，可以在站点之间运行的 WAN ISL 端口上配置 MACsec 加密。如果选择使用 MACsec 加密，则必须在应用正确的 RCF 文件后对其进行配置。

[NOTE]
====
* MACsec 加密只能应用于站点之间运行的 WAN ISL 端口。
* 配置 Cisco 9336C 交换机时，配置 MACsec 加密是可选的。
