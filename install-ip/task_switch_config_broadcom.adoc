...
NOTE: 在极少数情况下，交换机可能无法启动。按照  to upgrade EFOS using the ONIE OS installation 以安装新映像。
...
+
.. 配置交换机基本设置。按照步骤 4 及更高版本进行操作：  the Broadcom IP switch to factory defaults
.. 创建并应用 RCF 文件，如中所述  and installing the Broadcom RCF files
