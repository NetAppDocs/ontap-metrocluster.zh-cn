...
---
= 为 MetroCluster IP 配置配置 Cisco IP 交换机
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 您必须使用串行控制台连接到交换机。
* 此任务将重置管理网络的配置。
* 如果要将这些步骤作为交换机更换或升级等其他过程的一部分，请检查是否需要将集群 LIF 移动到伙伴交换机以启用无中断集群操作。link:task_install_and_cable_the_mcc_components.html["了解有关配置 MetroCluster IP 交换机的信息"].
...
.关于此任务
此任务要求使用 FTP、TFTP、SFTP、HTTP 或 SCP 等文件传输软件将文件复制到交换机。

* 如果要将这些步骤作为交换机更换或升级等其他过程的一部分，请检查是否需要将集群 LIF 移动到伙伴交换机以启用无中断集群操作。link:task_install_and_cable_the_mcc_components.html["了解有关配置 MetroCluster IP 交换机的信息"].
* 您必须使用支持的交换机软件版本。请参见 https://hwu.netapp.com["NetApp Hardware Universe"]。
...
. 在 MetroCluster IP 配置中其余的三个 IP 交换机上重复上述步骤。
+

NOTE: 您必须按以下顺序更新交换机：Switch_A_1、Switch_B_1、Switch_A_2、Switch_B_2。如果您的操作步骤要求您移动集群 LIF，则必须在配置第一个网络结构（Switch_A_1、Switch_B_1）中的交换机后，重复在第二个网络结构（Switch_A_2、Switch_B_2）上移动集群 LIF 的步骤。请参阅 link:task_install_and_cable_the_mcc_components.html["了解有关配置 MetroCluster IP 交换机的信息"]。
...
CAUTION: 您必须确保为集群 LIF 启用自动还原。

. 如果未启用自动还原，请修改所有集群 LIF 以启用自动还原：
+

NOTE: 如果这些步骤是作为其他过程（例如交换机更换或升级）的一部分执行的，则您可能已在执行 link:switch-move-cluster-lifs.html["移动集群 LIF"] 步骤时禁用了自动还原功能。
