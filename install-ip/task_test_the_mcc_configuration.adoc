...
+
... 确认所有节点均处于已配置状态和正常模式： `MetroCluster node show`
+
[listing]
----
...
----
... 开始切换操作： `MetroCluster switchover`
+
[listing]
----
...
----
... 确认本地集群处于已配置状态和切换模式： `MetroCluster node show`
+
[listing]
----
cluster_A::>  metrocluster node show
...
Remote: cluster_B               not-reachable          -
              configured             normal
----
... 确认切换操作已成功： `MetroCluster operation show`
+
[listing]
----
...
cluster_A::> metrocluster operation show
 ...
 Start Time: 2/6/2016 13:28:50
   End Time: 2/6/2016 13:29:41
     Errors: -
----
... 使用 vserver show 和 network interface show 命令验证 DR SVM 和 LIF 是否已联机。
...
. 如果系统运行的是 ONTAP 9.4 或更早版本，请修复数据聚合： `MetroCluster heal aggregates`
+
以下示例显示了命令的成功完成：
...
----
. 如果系统运行的是 ONTAP 9.4 或更早版本，请修复根聚合： `MetroCluster heal root-aggregates`
+
以下配置需要执行此步骤：
...
----
. 确认修复已完成： `MetroCluster node show`
+
以下示例显示了命令的成功完成：
...
----
+
如果自动修复操作因任何原因失败，您必须 MetroCluster 按照 ONTAP 9.5 之前的 ONTAP 版本中的步骤手动执行问题描述修复命令。您可以使用 MetroCluster operation show 和 MetroCluster operation history show -instance 命令监控修复状态并确定故障的发生原因。

. 验证所有聚合是否均已镜像： `storage aggregate show`
+
...
----
. 从灾难站点启动节点。
. 检查切回恢复的状态： `MetroCluster node show`
+
[listing]
----
cluster_A::> metrocluster node show
...
2 entries were displayed.
----
. 确认 MetroCluster 操作的状态： `MetroCluster operation show`
+
输出应显示成功状态。
...
.. 使用 system environment sensors show -state fault 和 storage shelf show -errors 命令在控制台上监控结果。
+
[listing]
----
...
node_A_2_root
           707.7GB   34.27GB   95% online       1 node_A_2       raid_dp,
                                                                   ...
                                                                   normal
----
.. 确认所有数据 SVM 和数据卷均已联机并提供数据： `vserver show -type data``network interface show -fields is-home false``volume show ！ vol0 ，！ mDV*`
+
[listing]
----
...
----
.. 在池 1 中为 node node_A_2 确定一个磁盘架以关闭电源以模拟突发硬件故障： `storage aggregate show -r -node node-name ！ * root`
+
...
15 entries were displayed.
----
.. 确认正在提供数据且所有卷仍处于联机状态： `vserver show -type data``network interface show -fields is-home false``volume show ！ vol0 ，！ mDV*`
+
[listing]
----
cluster_A::> vserver show -type data
...
.. 确认重新同步已启动： `storage aggregate show`
+
...
node_A_2_root
           707.7GB   34.27GB   95% online       1 node_A_2       raid_dp,
                                                                   resyncing
----
.. 监控聚合以确认重新同步已完成： `storage aggregate show`
+
