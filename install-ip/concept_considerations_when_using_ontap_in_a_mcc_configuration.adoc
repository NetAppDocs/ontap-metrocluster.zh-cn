...
有关使用 FabricPool 的常规信息，请参见 _Disks and aggregates management_ 。
...
NOTE: 在配置聚合之前，您必须按照 _Disks and aggregates management_ 中的 " 在 MetroCluster 配置中为 FabricPool 设置对象存储 " 中所述设置对象存储。
...
. 创建包含聚合的 FabricPool 镜像，如 _Disks and Aggregates management_ 中所述：
+
https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-psmg/home.html["磁盘和聚合管理"]
