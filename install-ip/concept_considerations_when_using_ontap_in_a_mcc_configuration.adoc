...
有关使用 FabricPool 的常规信息，请参见 https://["磁盘和层(聚合)管理"^]。
...
NOTE: 在配置聚合之前，必须按照中的 " 在 MetroCluster 配置中为 FabricPool 设置对象存储 " 所述设置对象存储 https://["磁盘和聚合管理"^]。
...
. 使用聚合创建 FabricPool 镜像，如中所述 https://["磁盘和聚合管理"^]
+
.. 附加主对象存储。
+
此对象存储在物理上更接近集群。
...
请参见中的 "`复制 SVM 配置` " http://["数据保护"^] 有关配置 SVM DR 关系的详细信息。
