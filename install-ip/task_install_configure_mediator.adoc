...
* 如果使用第三方防火墙：请参见 link:concept_mediator_requirements.html#firewall-requirements-for-ontap-mediator["ONTAP 调解器的防火墙要求"]
...
link:concept_mediator_requirements.html#guidelines-for-upgrading-the-ontap-mediator-in-a-metrocluster-configuration["在 MetroCluster 配置中升级 ONTAP 调解器的准则"]
...
* 适用于的操作步骤 <<centos8x,CentOS 8.0.1095 或更高版本>> 操作系统
...
. 运行 `yum repolist` 命令。
+
以下示例显示了此命令的执行情况。列表中应显示 "`CentOS-7.6.1810 - Base` " 存储库。
...
CAUTION: CentOS 8.x 的生命周期支持将于 2021 年 12 月 31 日结束。如果将 CentOS 与 ONTAP 结合使用，建议您还原到 <<centos7x,CentOS 7.x>> 操作系统或使用 RHEL 。

如果您的操作系统为 * CentOS 8.0.1905 或更高版本 * ：

在中 http://vault.centos.org["CentOS 存储"]下，找到与您的操作系统匹配的 kernel-devel 软件包。因为最新版本的 8.0 （ CentOS 8.0.1905 及更高版本）核心位于中，您必须提供对匹配的 kernel-devel 软件包的访问权限，以编译所需的内核模块。

.步骤
. 【 ｛ step1-rpm]] 直接安装 kernel-devel 软件包：
+
...
NOTE: 上述命令是一个特定于 CentOS 8.0.1905 的示例。根据其他 CentoOS 8.x 版本的需要更改路径。
