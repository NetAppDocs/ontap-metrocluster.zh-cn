...
. 验证切换后的数据聚合是否已切回： `storage aggregate show`
+
...
----
+
如果灾难站点包含未镜像聚合，而未镜像聚合不再存在，则该聚合可能会在 storage aggregate show 命令的输出中显示为 unknown 。请联系技术支持以删除未镜像聚合的过期条目。

. 验证运行正常的集群上的所有 sync-destination SVM 是否均处于休眠状态（显示 Admin State 为 stopped ），以及灾难集群上的 sync-source SVM 是否已启动并正在运行： `vserver show -subtype sync-source`
+
[listing]
----
...
. 使用 MetroCluster operation show 命令确认切回操作是否成功。
+
[cols="2*"]
|===
| 如果命令输出显示 ... | 那么 ... 
